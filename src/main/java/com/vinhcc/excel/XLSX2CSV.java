...
public class XLSX2CSV {
    ...
    private class SheetToCSV implements SheetContentsHandler {
        ...
        private CSVRowListener listeners = null;

        public void addListener(CSVRowListener object) {
            listeners = object;
        }
        ...
        private void outputMissingRows(int rowNum, int number) {
            for (int i = 0; i < number; i++) {
                ...
                if (this.listeners != null) {
                    this.listeners.ProcessRow(rowNum, cols);
                }
            }
        }
    ...
    public void process() throws IOException, OpenXML4JException, SAXException {
        ...
        while (iter.hasNext()) {
            try (InputStream stream = iter.next()) {
                ...
                //this.sb.println();
                //this.sb.println(sheetName + " [index=" + index + "]:");
                SheetToCSV csv = new SheetToCSV();
                ExcelImport imp = new ExcelImport(sheetName);
                csv.addListener(imp);
                
                processSheet(styles, strings, csv, stream);
            }
            ++index;
        }
    }
